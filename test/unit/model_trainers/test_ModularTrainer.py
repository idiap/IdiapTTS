...
class TestModularTrainer(unittest.TestCase):
    ...
    def test_split_return_values_torch(self):
        ...
        with unittest.mock.patch.object(ModularTrainer.logger, "error") as mock_logger:
            with self.assertRaises(TypeError):
                ModularTrainer._split_return_values(output, seq_length_output, None, False)
    ...
    def test_split_return_values(self):
        ...
        split_batch = ModularTrainer._split_return_values(batch, seq_length_output, None, False)
    ...
    def test_split_return_values_some_hidden_none(self):
        seq_length_output = numpy.array([10, 6, 8])
        batch_size = 3
        feature_dim = 50
        output = numpy.empty((seq_length_output.max(), batch_size, feature_dim))
        ...
        for idx in range(batch_size):
            ...
            self.assertTrue(h1 is None,
                            msg="Hidden1 of batch {} is wrong, expected was None.".format(idx))
            self.assertTrue((h2 == idx * 100).all(),
                            msg="Hidden2 of batch {} is wrong, expected was all values being {}."
                            .format(idx, idx * 100))
        ...
        for idx in range(batch_size):
            ...
            self.assertTrue((h2 is None),
                            msg="Hidden2 of batch {} is wrong, expected was None.".format(idx, idx * 100))
    ...
    def test_split_return_values_batch_first(self):
        ...
        split_batch = ModularTrainer._split_return_values(batch, seq_length_output, None, True)
    ...
    def test_input_to_str_list(self):
        ...
        out = ModularTrainer._input_to_str_list(os.path.join("integration", "fixtures", "file_id_list.txt"))
