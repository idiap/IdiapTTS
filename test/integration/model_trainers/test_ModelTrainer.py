...
#                 return [self.l2(output, target), self.l1(output, target)]
...
#         trainer = self._get_trainer(hparams)
#         trainer.loss_function = Multiloss()
#         # trainer.loss_function = torch.nn.MSELoss(reduction='none')
...
#         all_loss, all_loss_train, _ = trainer.train(hparams)
#         self.assertTrue(numpy.isclose(0.91936, all_loss[-1][0]))

#         trainer.get_losses_plot(save_to_file=True, hparams=hparams)
#         log_dir = os.path.join(hparams.out_dir, "log")
#         self.assertEqual(1, len([name for name in os.listdir(log_dir) if os.path.isfile(os.path.join(log_dir, name)) and name.endswith(".pdf")]))
...
#         target_contiguous = [element for element in trainer.model_handler.dataloader_val][0][1].view(1, -1)
