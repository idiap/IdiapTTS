...
split_file_id_list() {
    local file_id_list=${1:-${file_id_list}}
    local name_file_id_list=${2:-$(basename ${file_id_list})}

    if [ "$#" -lt 3 ]; then
        if [[ -z "$dir_out" ]]; then
            echo "ERROR: \$dir_out needs to be set or passed as 3rd argument."\
                "The script looks for \$dir_out/${name_file_id_list}"
            return 1
        fi
    else
        local dir_out="${3}"
    ...
    split --numeric=1 --suffix-length ${suffix_length} -l ${block_size}\
        ${file_id_list} ${dir_out}/${name_file_id_list}_block
...
cleanup_split_file_id_list() {
    local name_file_id_list=${1:-$(basename ${file_id_list})}
    local dir_out=${2:-${dir_out}}
...
downsample() {
    ...
    ./${cpu_1d_cmd} JOB=1:${num_blocks} ${dir_logs}/${name_file_id_list}_down_sampling_blockJOB.log \
                    ...
                    ${dir_audio}_org/ \
                    ${dir_audio}/ \
                    ${dir_out}/${name_file_id_list}_blockJOB \
                    ${target_frame_rate}
...
remove_silence() {
    local silence_db=${1:-"-30"}
    local min_silence_ms=${2:-"200"}
...
normalize_loudness() {
    ...
    ./${cpu_1d_cmd} JOB=1:${num_blocks} ${dir_logs}/${name_file_id_list}_loudness_normalisation_blockJOB.log \
    ...
    # Print errors and save warnings.
    eval grep --ignore-case "WARNING" "${dir_logs}/${name_file_id_list}_loudness_normalisation_block{1..${num_blocks}}.log" >| "${dir_logs}/${name_file_id_list}_loudness_normalisation_WARNINGS.txt"
    eval grep --ignore-case "ERROR" "${dir_logs}/${name_file_id_list}_loudness_normalisation_block{1..${num_blocks}}.log"
}

high_pass_filter() {

    local stop_freq_Hz=${1:-"70"}
    local pass_freq_Hz=${2:-"100"}
    local filter_order=${3:-"1001"}
...
single_channel_noise_reduction() {
    ...
    python ${dir_src}/data_preparation/audio/single_channel_noise_reduction.py \
                  ...
                  --dir_out ${dir_audio}/ \
                  --file_id_list ${dir_out}/${name_file_id_list}
