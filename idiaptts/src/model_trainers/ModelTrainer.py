...
class ModelTrainer(object):
    ...
    #     self.model_handler = ModelHandler()  # A handler for the NN models depending on the NN frameworks.
    ...
    #     self.losses = []  # Saves the validation and training loss of all calls of the train method.

    #     self.total_epoch = None  # Total number of epochs the current model was trained.

    # def _check_hparams(self, hparams: ExtendedHParams):
    ...
    #     self.logger.info("ModelTrainer ready.")
    ...
    #     self.model_handler.save_checkpoint(model_path, epoch=self.total_epoch)
    ...
    #         all_loss_train.append([-1.0] * len(best_loss))  # Set a placeholder at the train losses.
    #         all_loss.append(best_loss)
    #         self.trained_epochs.append(self.total_epoch)
    ...
    #                 if loss[0] < best_loss[0] or np.isnan(best_loss[0]):
    #                     best_loss = loss
    ...
    #                     self.model_handler.run_scheduler(loss[0], self.total_epoch + 1)

    #     t_training = timer() - t_start
    #     self.logger.info('Training time: ' + str(timedelta(seconds=t_training)))
    #     for loss_idx, (val_loss, train_loss) in enumerate(list(zip(zip(*all_loss), zip(*all_loss_train)))):
    #         self.logger.info('Loss {} validation progress: '.format(loss_idx)
    #                          + ', '.join('{:.4f}'.format(l) for l in val_loss))
    #         self.logger.info('Loss {} train progress: '.format(loss_idx)
    #                          + ', '.join('{:.4f}'.format(l) for l in train_loss))
    ...
    #     self.losses.append((np.array(all_loss_train), np.array(all_loss), np.array(self.trained_epochs)))
    #     # if len(all_loss) == 1:  # Backwards compatibility.
    #     #     all_loss = all_loss[0]
    #     #     all_loss_train = all_loss_train[0]
    #     return all_loss, all_loss_train, self.model_handler

    # def sanity_check_train(self, hparams):
    #     assert self.model_handler is not None, "The init function has been called before training."
    ...
    #     self.model_handler.set_dataset(hparams, self.dataset_train, self.dataset_val, self.batch_collate_fn)
    #     self.log_validation_set()
    #     loss, features_loss = self.model_handler.test(hparams, self.total_epoch, self.total_epoch, self.loss_function)
    #     for loss_idx, val_loss in enumerate(loss):
    #         self.logger.info('Loss {} validation: {:.4f}'.format(loss_idx, val_loss))

    #     return loss, features_loss
    ...
    # def get_losses_plot(self, save_to_file=False, hparams=None, file_path=None):
    #     colors = cycle(DataPlotter.get_color(2))
    #     fig, ax = plt.subplots(self.losses[0][0].shape[1], 1, squeeze=False)
    #     for idx, (train_loss, validation_loss, epochs) in enumerate(self.losses):
    #         for loss_idx in range(train_loss.shape[1]):
    #             mask = train_loss[:, loss_idx] > 0
    #             ax[loss_idx, 0].plot(epochs[mask], train_loss[mask, loss_idx],
    #                                      label="train" if idx == 0 and loss_idx == 0 else None, color=next(colors))
    #             ax[loss_idx, 0].plot(epochs, validation_loss[:, loss_idx],
    #                                      label="validation" if idx == 0 and loss_idx == 0 else None, color=next(colors))
    #             ax[loss_idx, 0].set_ylabel('loss {}'.format(loss_idx))
    #     # ax[-1].vlines(epochs[0], ymin=0, ymax=1)
    #     ax[-1, 0].set_xlabel('epochs')
    #     ax[0, 0].legend()
    #     if hparams is not None:
    #         fig.title = hparams.model_name

    #     if save_to_file:
    #         assert hparams is not None, "Save to file requires hparams to determine the default path."
    #         log_dir = os.path.join(hparams.out_dir, "log")
    #         makedirs_safe(log_dir)
    #         plt.savefig(os.path.join(log_dir, "{}_losses.pdf".format(os.path.splitext(hparams.model_name)[0])))
    ...
    #     t_start = timer()
    #     model_output, model_output_post = self._forward_batched(hparams, id_list, hparams.batch_size_gen_figure, load_target=False, synth=False, benchmark=False, gen_figure=True)
    #     t_training = timer() - t_start
    ...
    # def gen_output(self, hparams, ids_input):
    #     """
    #     Generate figures for all given ids with the self.gen_figure_from_output function (has to be implemented).
    ...
    #     assert self.model_handler is not None, "No model_handler exists, was trainer.init() called before."
    ...
    #     t_start = timer()
    #     model_output, model_output_post = self._forward_batched(hparams, id_list, hparams.batch_size_gen_figure, load_target=False, synth=False, benchmark=False, gen_figure=False)

    #     for key, output in model_output_post.items():
    #         self.OutputGen.save_output(output, hparams.save_output_dir, key)

    #     t_training = timer() - t_start
    #     self.logger.info('Output generation time for {} sample(s): {}'.format(len(id_list), timedelta(seconds=t_training)))
    ...
    #             batch_input_labels, batch_target_labels, seq_length_inputs, seq_length_output, _, permutation, *extra_labels = self.batch_collate_fn(inputs, common_divisor=hparams.num_gpus, batch_first=hparams.batch_first)
    #         else:
    #             batch_input_labels, batch_target_labels, seq_length_inputs, seq_length_output, _, permutation, *extra_labels = self.model_handler.prepare_batch(inputs, common_divisor=hparams.num_gpus, batch_first=hparams.batch_first)

    #         # Run forward pass of model.
    #         nn_output, nn_hidden = self.model_handler.forward(batch_input_labels, hparams, seq_length_inputs, target=batch_target_labels, extra_labels=extra_labels)
    ...
    # def synthesize(self, file_id_list, synth_output, hparams, use_model_name=True, has_deltas=False,
    ...
    #     if hparams.synth_vocoder == "WORLD":
    #         Synthesiser.run_world_synth(synth_output, hparams, use_model_name=use_model_name, has_deltas=has_deltas,
    ...
    #         Synthesiser.run_r9y9wavenet_mulaw_world_feats_synth(synth_output, hparams, epoch=epoch, step=step)
    ...
    #         Synthesiser.run_raw_synth(synth_output, hparams, epoch=epoch, step=step)
